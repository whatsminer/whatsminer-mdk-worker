...
const configuredPoolView = (pools) => Array.isArray(pools)
  ? pools.slice(0, 3).filter(pool => pool?.url).map((pool, index) => ({
    index: index + 1,
    url: pool.url,
    user: pool.worker_name || pool.user || pool.account || ''
  }))
  : []
...
class WhatsminerApiV3Client {
  constructor (opts) {
    ...
    this.configuredPools = configuredPoolView(this.conf.pools)
    this.lastSnap = null
    this._lastSeen = null
  ...
  async getPools () {
    ...
    const runtimePools = pools.map(pool => ({
      index: pool.id || 0,
      url: pool.url || '',
    ...
    if (this.configuredPools.length === 0) return runtimePools

    const used = new Set()
    const configured = this.configuredPools.map((pool, index) => {
      const runtimeIndex = runtimePools.findIndex((candidate, candidateIndex) => (
        !used.has(candidateIndex) && candidate.url === pool.url
      ))
      if (runtimeIndex === -1) {
        return {
          ...pool,
      ...
      return {
        ...pool,
        ...runtimePools[runtimeIndex],
        index: index + 1,
        user: runtimePools[runtimeIndex].user || pool.user
      }
    })

    return configured.concat(runtimePools.filter((pool, index) => !used.has(index)))
  }
  ...
  async setPools (pools, appendId = true) {
    ...
    try {
      ...
      this.configuredPools = param.map((pool, index) => ({
        index: index + 1,
        url: pool.pool,
        user: pool.worker
      }))
      delete this.deviceDataCache.getPools
      return { success: true }
    } catch (error) {
      return { success: false, error_msg: error.message }
    }
